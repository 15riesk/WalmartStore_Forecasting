...
Walmart is an American multinational retail corporation that operates a chain of hypermarkets and grocery stores in the United States, headquartered in Bentonville, Arkansas. Through Kaggle, Walmart opened a recruiting competition where job-seekers are provided with historical sales data for 45 Walmart stores across the country each containing many departments. Additionally, Walmart runs promotional markdown events throughout the year. These markdowns precede holidays, the four largest of which are the Super Bowl, Labor Day, Thanksgiving, and Christmas. The objective is to predict the sales for each department in each store (~3,000 Forecasts) using Machine Learning Algorithms.  
...
The reason that the model is evaluated through a WMAE is that weeks including holidays are weighted five times higher in the evaluation than non-holiday weeks. During holidays $w$ is equal to 5, else $w$ is equal to 1.
...
We observe that all stores follow a similar trend, with an inflection plot towards the end of the year.
...
Now that we have performed Time Series Decomposition and seen the effects that Trend, Seasonality and Remainders have on our time series, we will dive into our features and determine through Feature Selection which features influence Weekly Sales, our target variable, the most.
...
In a single statement, Random Forest is an ensemble (combined predictions from two or more models) of Decision Tree algorithms.
...
In building our Machine Learning Algorithms, we will use the help of a library Modeltime. Modeltime allows the user to develop multiple Machine Learning Algorithms for Time Series Forecasting. The first steps in using Modeltime is data preparation. Here, we first extend our time series as far as we wish to predict. Next, we identify our ID column which acts as our "grouper" which is StoreDept and again restate our extension. Finally, as we stated in the beginning of this project, we split our data into a Train and Validation set. We split by equal parts, 53 weeks, ensuring that we have a full year to fit our models.
...
We see a massive improvement in our results from Multiple Linear Regression to Random Forest.
...
The final results are in. We see a slight decrease in accuracy from Random Forest but overall, both models outperformed our Linear Regression Model.
